...
class CommandRunner {
  // a constructor that accepts the optional callback
  ...
  ArgResults parse(List<String> input) {
    ...
    if (_commands.containsKey(input.first)) {
      ...
      throw ArgumentException(
        'The first word of input must be a commmand.',
        null,
        input.first,
      );
    }
    ...
    while (i < input.length) {
      if (input[i].startsWith('-')) {
        ...
        // Throw an exception if the option is not defined for the command
